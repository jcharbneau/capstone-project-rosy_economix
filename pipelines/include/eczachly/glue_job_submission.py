...
def create_glue_job(
    ...
    **kwargs
    ...
    output_table = kwargs['dag_run'].conf.get('output_table', arguments.get('--output_table', ''))
    ...
    job_args = {
        ...
        'DefaultArguments': {
            ...
            + f'--conf spark.sql.defaultCatalog={catalog_name} ' +
            f'--conf spark.sql.catalog.{catalog_name}=org.apache.iceberg.spark.SparkCatalog '
            +
            f'--conf spark.sql.catalog.{catalog_name}.credential={tabular_credential} '
            + '--conf spark.sql.shuffle.partitions=50',
            "--extra-jars":
            f"s3://{s3_bucket}/jars/iceberg-spark-runtime-3.3_2.12-1.5.2.jar,s3://{s3_bucket}/jars/iceberg-aws-bundle-1.5.2.jar"
        },
        'GlueVersion': '4.0',
        'WorkerType': 'Standard',
        'NumberOfWorkers': 1
